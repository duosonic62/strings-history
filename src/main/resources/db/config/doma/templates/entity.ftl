...
<#-- CommonEntity を継承 -->
public class <#if entityPrefix??>${entityPrefix}</#if>${simpleName}<#if entitySuffix??>${entitySuffix}</#if> extends CommonEntity {
<#list ownEntityPropertyDescs as property>
<#-- CommonEntity に定義されているカラムは除外 -->
  <#if property.name != "id" && property.name != "createdAt" && property.name != "updatedAt" && property.name != "isDeleted" && property.name != "version">
    ...
    <#if !useAccessor>
      public </#if>${property.propertyClassSimpleName} ${property.name};
  </#if>
</#list>
